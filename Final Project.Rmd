...
```{r echo=F}
  ...
  ggplot(appleStore_cat, aes(x = prime_genre, y = user_rating)) +
...
```{r }
  ggplot(appleStore_cat, aes(x = cont_rating, y = user_rating)) +
...
```{r }
  ...
  ggplot(appleStore_cat, aes(x = PriceCat, y = user_rating)) +
      geom_boxplot() +
      #ggtitle("Boxplot of User Rating by Genre across Free Vs Paid Apps")
      theme_bw() +
      theme(plot.title = element_text(hjust = 0.5)) +
...
```{r echo = F}
ggplot(freeApps, aes(prime_genre)) +
...
```{r echo = F}
ggplot(paidApps, aes(prime_genre)) +
  geom_bar(fill = viridis(23), color = 'black') +
...
```{r echo = F}
catFreeRating = aggregate(freeApps$user_rating, list(freeApps$prime_genre), mean)
...
```{r echo = F}
catPaidRating = aggregate(paidApps$user_rating, list(paidApps$prime_genre), mean)
...
```{r echo = F}
ggplot(freeApps, aes(cont_rating)) +
...
```{r echo = F}
ggplot(paidApps, aes(cont_rating)) +
  geom_bar(fill = viridis(4), color = 'black') +
