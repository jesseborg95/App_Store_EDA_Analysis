...
plotNA <- barplot(sapply(playstore,function(x)sum(is.na(x))),
                  col = "lightblue",
...
ratingNA <- subset(playstore, is.na(Rating))
playstore = na.omit(playstore)

# to double check that the NAs have been removed
sapply(playstore,function(x)sum(is.na(x)))
```
...
distinct <- nrow(playstore %>%
  distinct())

duplicates <- nrow(playstore) - distinct
```
...
playstore=playstore[!duplicated(playstore), ]
