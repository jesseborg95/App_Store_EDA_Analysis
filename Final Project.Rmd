...
## Price
``` {r type, echo = F}
freeApps <- subset(appleStore, appleStore$Price == 0.00)
paidApps <- subset(appleStore, appleStore$Price != 0.00)
...
```{r echo=F}
  appleStore_cat <- appleStore[which(appleStore$Price == 0.00 |
                                       appleStore$Price != 0.00
                                       ), ]
  appleStore_cat$PriceCat <- factor(ifelse(appleStore_cat$Price == 0.00, 0, 1), labels = c("Free", "Paid"))
  ggplot(appleStore_cat, aes(x = Category, y = Rating)) +
    geom_boxplot(fill=viridis(46)) +
...
```{r }
  ggplot(appleStore_cat, aes(x = Content_Rating, y = Rating)) +
      geom_boxplot(fill=viridis(8)) +
      #ggtitle("Boxplot of User Rating by Genre across Free Vs Paid Apps")
      theme_bw() +
      theme(plot.title = element_text(hjust = 0.5)) +
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
...
```{r }
  ...
  ggplot(appleStore_cat, aes(x = PriceCat, y = Rating)) +
      geom_boxplot() +
      #ggtitle("Boxplot of User Rating by Genre across Free Vs Paid Apps")
      theme_bw() +
      theme(plot.title = element_text(hjust = 0.5)) +
...
```{r echo = F}
ggplot(freeApps, aes(Category)) +
  geom_bar( fill = viridis(23), color = 'black') +
  ggtitle('Free Apps in the appleStore by Genre') + 
  xlab('Genre') + ylab('Number of Apps') + 
...
```{r echo = F}
ggplot(paidApps, aes(Category)) +
  geom_bar(fill = viridis(23), color = 'black') +
  ggtitle('Paid Apps in the appleStore by Genre') + 
  xlab('Genre') + ylab('Number of Apps') + 
  coord_cartesian(ylim = c(0, 1000)) + 
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 8, color = 'black'))
```
In the appleStore, apps are either free or paid. 
What genre of apps are rated more in the free vs paid apps?

```{r echo = F}
catFreeRating = aggregate(freeApps$Rating, list(freeApps$Category), mean)
names(catFreeRating) = c("Genre", "Avg_rating")

ggplot(catFreeRating, aes(x = Genre, y = Avg_rating)) + 
  geom_bar(stat = 'Identity', fill = viridis(23), color = "black") +
  ggtitle("Average Rating of Free Apps by Genre") +
   ylab('Average Rating') +
...
names(catPaidRating) = c("Genre", "Avg_rating")

ggplot(catPaidRating, aes(x = Genre, y = Avg_rating)) + 
  geom_bar(stat = 'Identity', fill = viridis(23), color = "black") +
  ggtitle("Average Rating of Paid Apps by Genre") +
   ylab('Average Rating') +
...
####TODO : Top 5 rated genre in Free Apps 
####TODO : Top 5 rated genre in Paid Apps 

```{r echo = F}
ggplot(freeApps, aes(Content_Rating)) +
  geom_bar(fill = viridis(4), color = 'black') +
  ggtitle('Free Apps in the appleStore by User Rating') + 
  xlab('Genre') + ylab('Number of Apps') + 
  coord_cartesian(ylim = c(0, 1000)) + 
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
...
```{r echo = F}
ggplot(paidApps, aes(Content_Rating)) +
  geom_bar(fill = viridis(4), color = 'black') +
  ggtitle('Paid Apps in the appleStore by User Rating') + 
