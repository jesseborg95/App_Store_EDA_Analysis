...
Furthermore, we examined how the apps are influenced by price in the next section. We observed the top rated categories among free are respectively: Events, Education, Books and Reference, Parenting and Art & Design; while those of paid apps include: News & Magazines, Education, Art & Design, Auto & Vehicles and Entertainment. In addition, we compared the average ratings of apps across different categories and discovered there was significant difference in the average ratings across the different app categories (Cheap, Expensive and Very Expensive)

Finally, the preceding section explores relationships between different numerical variables in Google PlayStore. It was interesting to find out that the variables not closely correlated. An attempt to fit different linear models with the rating variable as the target variable produced low values of coefficient of determination (r2), implying a only small variation in rating can be explained by a variation in the other variables (price and number of installs) in the model.
